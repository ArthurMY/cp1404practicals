    ...
    while choice != "Q":
        if choice == "L":
            ...
            for project in class_projects:
                print(
                    f"{project.name}\t{project.start_date}\t{project.priority}\t{project.cost}\t"
            ...
            cost = validate_cost_estimate()
            completion_percentage = validate_percentage("Percent Complete: ")
            class_projects.append(Project(name, start_date, priority, cost, completion_percentage))
        elif choice == "U":
            for i, project in enumerate(class_projects, 0):
                print(
                    f"{i} {project.name}, start:{project.start_date}, priority {project.priority}, estimate: "
                    f"${project.cost}, completion: {project.completion_percentage}% ")
...
def update_project(new_percentage, new_priority, project_choice):
    class_projects[project_choice] = Project(class_projects[project_choice].name,
                                             ...
                                             float(class_projects[project_choice].cost),
                                             int(new_percentage))
